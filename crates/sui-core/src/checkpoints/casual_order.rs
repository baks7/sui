...
struct RWLockDependencyBuilder {
    read_version: HashMap<ObjectVer, Vec<TransactionDigest>>,
    overwrite_versions: HashMap<TransactionDigest, Vec<ObjectVer>>,
}

type ObjectVer = (ObjectID, SequenceNumber);

impl RWLockDependencyBuilder {
    pub fn from_effects(effects: &[TransactionEffects]) -> Self {
        let mut read_version: HashMap<ObjectVer, Vec<TransactionDigest>> = Default::default();
        let mut overwrite_versions: HashMap<TransactionDigest, Vec<ObjectVer>> = Default::default();

...
async fn get_gas_status(
    ...
    let gas_object_ref = transaction.gas_payment_object_ref().get(0).unwrap();
    ...
    let mut remaining_gas_object_refs: Vec<ObjectRef> = transaction.gas_payment_object_ref()[1..].to_vec();
